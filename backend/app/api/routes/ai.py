...
def chat(request: AIChatRequest, db: Session = Depends(get_db)):
    ...
    is_time_selection = any(t in user_msg_lower for t in time_slots)
    ...
    if not response_text:
        ...
        # After AI response, check if user provided date/time and slot is unavailable
        if "not available" not in response_text.lower():
            booking_data = extract_booking_details_from_history(history)
            if booking_data.get('preferred_date') and booking_data.get('preferred_time') and not booking_data.get('confirmed'):
                try:
                    parsed_date = parse_date(booking_data.get('preferred_date', ''))
                    if parsed_date:
                        time_str = parse_time_to_24h(booking_data.get('preferred_time', ''))
